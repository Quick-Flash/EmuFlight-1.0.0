...
static bool accNeedsCalibration(void)
{
    if (sensors(SENSOR_ACC)) {
        ...
        if (isModeActivationConditionPresent(BOXANGLE) ||
            ...
            isModeActivationConditionPresent(BOXAUTOTUNE) ||
...
bool processRx(timeUs_t currentTimeUs)
{
    ...
    if (IS_RC_MODE_ACTIVE(BOXAUTOTUNE)) {
        ...
        if (!FLIGHT_MODE(AUTOTUNE)) {
            ENABLE_FLIGHT_MODE(AUTOTUNE);
        }
    } else {
        DISABLE_FLIGHT_MODE(AUTOTUNE);
    }

    if (((IS_RC_MODE_ACTIVE(BOXANGLE) && canUseAngleMode) || failsafeIsActive()) && (sensors(SENSOR_ACC))) {
    ...
    if (FLIGHT_MODE(ANGLE_MODE) || FLIGHT_MODE(HORIZON_MODE) || FLIGHT_MODE(AUTOTUNE)) {
        LED1_ON;
