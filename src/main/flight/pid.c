...
void FAST_CODE pidController(const pidProfile_t *pidProfile, timeUs_t currentTimeUs)
{
    ...
    for (int axis = FD_ROLL; axis <= FD_YAW; ++axis) {
        ...
        if ((pidRuntime.pidCoefficient[axis].Kd > 0) && !launchControlActive) {
            ...
            const float pureMeasurement = -(gyro.gyroADCf[axis] - pidRuntime.previousMeasurement[axis]);
            pidRuntime.previousMeasurement[axis] = gyro.gyroADCf[axis];
            float dDelta = pureMeasurement * pidRuntime.pidFrequency;
            // Filter dterm properly
            dDelta = pidRuntime.dtermNotchApplyFn((filter_t *) &pidRuntime.dtermNotch[axis], dDelta);
            dDelta = pidRuntime.dtermLowpassApplyFn((filter_t *) &pidRuntime.dtermLowpass[axis], dDelta);
            dDelta = pidRuntime.dtermLowpass2ApplyFn((filter_t *) &pidRuntime.dtermLowpass2[axis], dDelta);
            ...
            if (pidRuntime.dMinPercent[axis] > 0) {
                float dMinGyroFactor = biquadFilterApply(&pidRuntime.dMinRange[axis], dDelta);
...
#endif
            pidData[axis].D = pidRuntime.pidCoefficient[axis].Kd * dDelta * tpaFactor * dMinFactor;
        } else {
            pidData[axis].D = 0;
        }
